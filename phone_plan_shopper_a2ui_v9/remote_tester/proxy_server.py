...
ENGINE_ID = os.environ.get("ENGINE_ID") or os.environ.get("REASONING_ENGINE_ID")
...
async def handle_jsonrpc(request: Request):
    ...
    if method == "message/send":
        ...
        if res.status_code != 200:
            logger.error(f"Remote agent error: status={res.status_code}, response={res.text}")
            ...
            for _ in range(30): # Increase to 30 retries (60 seconds)
                ...
                if task_res.status_code == 200:
                    ...
                    if state == "TASK_STATE_SUCCEEDED" or state == "TASK_STATE_COMPLETED":
                        ...
                        logger.error(f"Task failed. Task data: {json.dumps(task_data)}")
                        return {"jsonrpc": "2.0", "error": {"code": 500, "message": f"Task failed: {task_data.get('status', {}).get('message', 'Unknown error')}"}, "id": request_id}
                else:
