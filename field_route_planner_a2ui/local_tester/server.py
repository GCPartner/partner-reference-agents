...
# Add project root to path to import packages correctly
...
import field_route_planner_a2ui.agent as agent
...
async def handle_jsonrpc(request: Request):
    ...
    if method == "message/send":
        ...
        # Append state to query to maintain context (Transcript Echoing)
        ...
        logger.info(f"Final agent response: {final_response_content}")
        ...
        try:
            ...
            logger.warning(f"Initial JSON parse failed: {e}. Trying to unescape backslash-n.")
            try:
                fixed_json = json_string_cleaned.replace('\\n', '\n')
                ui_data = json.loads(fixed_json)
                logger.info("Successfully parsed UI JSON after replacing escaped newlines.")
            except Exception as e2:
                logger.error(f"Failed to parse UI JSON: {e2}")
                logger.error(f"Raw json_string_cleaned was:\n{json_string_cleaned}")
                ui_data = []
