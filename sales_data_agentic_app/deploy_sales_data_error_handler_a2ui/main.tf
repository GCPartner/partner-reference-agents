...
locals {
  registration_payload = jsondecode(file("../${var.agent_folder_name}/registration_payload.json"))
...
module "agent_engine" {
  ...
  agent_engine_config = {
    ...
    environment_variables = {
      PROJECT_ID                                         = var.project_id
      LOCATION                                           = var.region
      SOURCE_BUCKET                                      = "${var.project_id}-sales-data"
      ERROR_BUCKET                                       = "${var.project_id}-sales-data-errors"
