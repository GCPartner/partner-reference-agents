...
class AdkAgentToA2AExecutor(agent_execution.AgentExecutor):
    ...
    async def execute(self, context: agent_execution.RequestContext, event_queue: events.EventQueue) -> None:
        ...
        logger.info(f"[DEBUG] execute() starting. context_id={context_id}, task_id={task_id}")
        
        # We will use a dictionary to accumulate recovered metadata keys.
        ...
        logger.info(f"[DEBUG] query={query}, initial metadata_dict={metadata_dict}")
        logger.info(f"[DEBUG] hasattr(context, 'message')={hasattr(context, 'message')}")
        
        # 1. SESSION RECOVERY: Extract state from A2UI payload
        try:
            if hasattr(context, 'message') and context.message:
                logger.info(f"[DEBUG] context.message type={type(context.message)}, dir={dir(context.message)}")
                parts = []
                ...
                logger.info(f"[DEBUG] Found {len(parts)} parts in context.message")
                for i, part in enumerate(parts):
                    logger.info(f"[DEBUG] Part {i} type={type(part)}, dir={dir(part)}")
                    data = None
                    if hasattr(part, 'data') and part.data:
                        data = part.data
                        logger.info(f"[DEBUG] Part {i} has .data attribute: type={type(data)}")
                    elif isinstance(part, dict) and 'data' in part:
                        data = part['data']
                        logger.info(f"[DEBUG] Part {i} is dict and has 'data' key")
                    elif hasattr(part, 'root') and hasattr(part.root, 'data'):
                        data = part.root.data
                        logger.info(f"[DEBUG] Part {i} has .root.data attribute: type={type(data)}")
                    else:
                        logger.info(f"[DEBUG] Part {i} fields: {part.__dict__ if hasattr(part, '__dict__') else str(part)}")
                        
                    if data:
                        logger.info(f"[DEBUG] Part {i} raw data content: {data}")
                        # Convert protobuf Struct to dict if it's a Protobuf message
                        if not isinstance(data, dict):
                            try:
                                from google.protobuf.json_format import MessageToDict
                                data = MessageToDict(data)
                                logger.info(f"[DEBUG] Converted Struct to dict successfully: {data}")
                            except Exception as ex:
                                logger.warning(f"[DEBUG] Struct conversion failed: {ex}")
                        ...
                        if isinstance(data, dict) and 'userAction' in data:
                            ...
                            logger.info(f"[DEBUG] Found userAction! Override query={query}")
                            # Recover Form Inputs
                            for item in user_action.get('inputs', []):
                                if item.get('id'): 
                                    metadata_dict[item['id']] = item['value']
                                    logger.info(f"[DEBUG] Recovered input: {item['id']}={item['value']}")
                            ...
                            for k, v in action_ctx.items():
                                if k != 'message':
                                    ...
                                    logger.info(f"[DEBUG] Recovered context key: {path_key}={v}")
                            ...
                            if metadata_dict.get('/trip/same_as_start') is True or metadata_dict.get('same_as_start') is True:
                                ...
                                if start_val:
                                    ...
                                    logger.info(f"[DEBUG] Server-side checkbox sync: copied start_location '{start_val}' to end_location")
        except Exception as e: logger.warning(f"Recovery failed: {e}", exc_info=True)
        ...
        logger.info(f"Running ADK Agent with query: {query}")
