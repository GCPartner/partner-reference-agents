...
async def run_e2e_test():
    print("Starting Playwright E2E Remote Browser Test...")
    screenshot_path = "/Users/veermuchandi/.gemini/antigravity-ide/scratch/remote_e2e_browser_result.png"
    ...
    async with async_playwright() as p:
        ...
        # Navigate to remote tester served on port 8000
        print("Navigating to http://localhost:8000...")
        await page.goto("http://localhost:8000")
        
        # Verify page loaded
        await page.wait_for_selector("text=A2UI Remote Tester")
        ...
        await page.wait_for_selector('[id="/trip/start_location_input"]', timeout=180000)
        ...
        # Print subsequent Agent messages for debugging
        await asyncio.sleep(5)
        agent_msgs = await page.locator('div:has-text("Agent:")').all_text_contents()
        print("Agent Messages after clicking Start Planning:")
        ...
        try:
            await map_iframe_locator.wait_for(state="attached", timeout=180000)
            print("Google Maps directions iframe rendered successfully!")
        except Exception as ex:
            print(f"Wait for iframe failed: {ex}")
            # Print page HTML
        ...
        # Log WebFrameUrl source URL
        try:
            iframe_src = await map_iframe_locator.get_attribute("src")
            print(f"MAP IFRAME SRC URL: {iframe_src}")
        except Exception as ex_inner:
            print(f"Failed to get map iframe src: {ex_inner}")
        
        # DEBUG: Print DOM text contents to see if it is there
        chat_text = await page.locator('#chat').inner_text()
        print("Chat box Text content:")
        print(chat_text)
        ...
        # Scroll the map iframe into view to force Chromium to render and paint it
        print("Scrolling map iframe into view...")
        await map_iframe_locator.scroll_into_view_if_needed()
        ...
        # Dump exact chat HTML
        chat_html = await page.locator('#chat').evaluate("el => el.outerHTML")
        with open("/Users/veermuchandi/.gemini/antigravity-ide/scratch/chat_dump.html", "w") as f:
            f.write(chat_html)
        print("Chat HTML dumped successfully to scratch/chat_dump.html")
