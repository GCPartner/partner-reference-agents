        ...
        try:
            async for event in runner.run_async(
                ...
                for resp in event.get_function_responses():
                    logger.info(f"[DEBUG Local Server] Function Response: name={resp.name}, response={resp.response}, type={type(resp.response)}")
                    val = None
                    ...
                    if isinstance(val, str) and "---a2ui_JSON---" in val:
                        logger.info(f"[DEBUG Local Server] Intercepted A2UI tool output: {val}")
