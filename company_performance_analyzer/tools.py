...
def generate_chart_ui(
    ...
    try:
        ...
        # Upload HTML to GCS
        bucket_name = "partner-engg-agents-adk-staging" # default fallback
        if csv_path.startswith("gs://"):
            parts = csv_path[5:].split("/")
            bucket_name = parts[0]
            
        import time
        import uuid
        session_suffix = str(uuid.uuid4())[:8]
        object_name = f"charts/chart_{chart_type}_{int(time.time())}_{session_suffix}.html"
        ...
        try:
            ...
            chart_url = f"https://storage.googleapis.com/{bucket_name}/{object_name}"
            logger.info(f"Successfully uploaded chart to GCS: {chart_url}")
        except Exception as upload_err:
            logger.error(f"Failed to upload chart to GCS: {upload_err}")
            chart_url = f"https://storage.googleapis.com/{bucket_name}/{object_name}"
