    ...
    for part in llm_response.content.parts:
        ...
        if "---a2ui_JSON---" in text:
            ...
            try:
                ...
                logger.error(f"Error parsing A2UI JSON in callback: {e}")
                new_parts.append(part)
        else:
            new_parts.append(part)
            
    if has_a2ui:
        logger.info("A2UI callback successfully wrapped A2UI parts.")
        return LlmResponse(
...
def validate_a2ui(parsed_json: Any, version: str = "v0.9"):
    """Validates the parsed A2UI payload against schema."""
