...
def main():
    ...
    # Use updateMask to target only the sharingConfig field
    url_with_mask = f"{api_endpoint}?updateMask=sharingConfig"
    ...
    print(f"Updating sharing config to ALL_USERS for agent {agent_id}...")
    print(f"Endpoint: {url_with_mask}")
    
    response = requests.patch(url_with_mask, headers=headers, json=payload)

    if response.status_code in [200, 201]:
         print("✓ Agent sharing config updated successfully!")
