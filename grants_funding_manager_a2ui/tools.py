...
def generate_submission_package(draft_text: str, target_grant_id: str, tool_context: ToolContext) -> dict:
    ...
    # Construct A2UI JSON in Python to guarantee valid syntax
    a2ui_data = [
        {"beginRendering": {"surfaceId": "review", "root": "review_card"}},
        {
            "surfaceUpdate": {
                ...
                "components": [
                    ...
                    {"id": "review_col", "component": {"Column": {"children": {"explicitList": ["review_title", "grant_details", "draft_lbl", "draft_preview", "package_summary_title", "attachments_included", "readiness", "submission_id", "word_count", "submit_btn_row"]}}}},
                    {"id": "review_title", "component": {"Text": {"text": {"literalString": "Grant Submission Package"}, "usageHint": "h2"}}},
                    {"id": "grant_details", "component": {"Text": {"text": {"literalString": f"Target Grant: {target_grant_id} - Public Health AI Innovation Grant"}, "usageHint": "h3"}}},
                    {"id": "draft_lbl", "component": {"Text": {"text": {"literalString": "Application Draft Preview:"}, "usageHint": "h4"}}},
                    {"id": "draft_preview", "component": {"Text": {"text": {"literalString": draft_text}}}},
                    {"id": "package_summary_title", "component": {"Text": {"text": {"literalString": "Package Summary:"}, "usageHint": "h4"}}},
                    {"id": "attachments_included", "component": {"Text": {"text": {"literalString": "Attachments Included: Audited Financials (SAP), Key Personnel Resumes (Workday), Strategic Alignment Memo"}}}},
                    {"id": "readiness", "component": {"Text": {"text": {"literalString": "Readiness: Ready for Executive Review"}}}},
                    {"id": "submission_id", "component": {"Text": {"text": {"literalString": f"Submission ID: SUB-{target_grant_id}-999"}}}},
                    {"id": "word_count", "component": {"Text": {"text": {"literalString": f"Word Count: {len(draft_text.split())}"}}}},
    ...
    tool_context.state["a2ui_json"] = json.dumps(a2ui_data)
    
    return {
        "status": "success", 
        "package_summary": package_summary, 
        "compiled_draft_preview": draft_text[:200] + "..."
    }
...
def send_a2ui_to_client(a2ui_json: str, tool_context: ToolContext) -> dict:
    """Send A2UI JSON to the client for rendering.
    
    Args:
        a2ui_json: The strictly valid A2UI JSON string.
    """
    try:
        # Validate JSON syntax
        json.loads(a2ui_json)
        # Save to state for server to pick up
        tool_context.state["a2ui_json"] = a2ui_json
        logger.info("Saved A2UI JSON to state via tool.")
        return {"status": "success", "message": "A2UI payload accepted and queued for delivery."}
    except Exception as e:
        logger.error(f"Tool failed to parse UI JSON: {e}")
        return {"status": "error", "message": f"Invalid JSON syntax: {e}. Please correct the JSON and try again."}
