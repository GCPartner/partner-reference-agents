"""Agent executor for ADK agents with A2UI validation (adapted for careconnect_navigator_a2ui)."""
...
class AdkAgentToA2AExecutor(agent_execution.AgentExecutor):
  ...
  async def execute(
    ...
    # Extract action context from DataPart if available (handles both action and userAction)
    try:
    ...
    while attempt <= max_retries:
      ...
      if "---a2ui_JSON---" not in final_response_content:
        ...
        try:
          ...
          version = a2ui_utils.get_a2ui_version()
          a2ui_utils.validate_a2ui(parsed_json, version)
          is_valid = True
        except Exception as e:
      ...
      if is_valid:
        ...
        if isinstance(json_data, dict):
            if "messages" in json_data:
                messages_to_send = json_data["messages"]
